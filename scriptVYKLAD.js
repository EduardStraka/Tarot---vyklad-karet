...
function drawOneCard() {
    ...
    displayCard(randomCard, [-2000]);
}

function drawThreeCards() {
    const selectedCards = getRandomCards(3);
    selectedCards.forEach((card, index) => {
        displayCard(card, -1240); // Zde používáme pevné hodnoty pro pozice karet
    });
}
function drawFourCards() {
    const selectedCards = getRandomCards(4);
    displayCards(selectedCards, [-580, -800, -580, -900], [10, 20, 20, 20]);
}
...
function redirectToVyznam(karta) {
    window.location.href = 'indexVYZNAM.html?' + encodeURIComponent(karta);
}
// Funkce pro skrytí karet
function hideCards() {
    const cardContainer = document.getElementById('karty');
    cardContainer.innerHTML = ''; // Vymažeme obsah karty
}

// Přidání posluchače události kliknutí na kontejner karet
const cardContainer = document.getElementById('karty');
cardContainer.addEventListener('click', function(event) {
    event.stopPropagation(); // Zabraňuje propagaci události kliknutí, aby nedošlo k zavření karet
    

    // Pokud kliknutý prvek není součástí kontejneru karet a není součástí hlavního obsahu stránky, skryjeme karty
    if (!cardContainer.contains(event.target) && !mainContent.contains(event.target)) {
        hideCards();
    }
});
